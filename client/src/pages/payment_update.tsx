...
const Payment_update: React.FC<PaymentUpdateProps> = ({
  ...
  const getPaymentRecords = async () => {
    try {
      const response = await axios.get<payment[]>(
        `${VITE_DATABASE_HOST}/payment-records/rekod-pembayaran/${payment_id}`
      );
  ...
  const deleteProgram = async () => {
    try {
      await axios.delete(
        `${VITE_DATABASE_HOST}/payment-records/rekod-pembayaran/${payment_id}/delete`
      );
  ...
  const onSubmit: SubmitHandler<any> = async (data) => {
    ...
    axios
      .put(
        `${VITE_DATABASE_HOST}/payment-records/rekod-pembayaran/${payment_id}/edit`,
        formData,
        {
      ...
      .then((response) => {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            getPaymentRecords(); // Refresh the payment details after update
          ...
          <div>
            ...
            <h2 className="text-lg mb-4">
              ...
              <span className="text-large font-semibold">
                {dayjs(payment?.records_timeStamp).format("DD/MM/YYYY hh:mm A")}
              </span>
            </h2>
            <h2 className="text-lg mb-4">
              Direkod Oleh :{" "}
                ...
                <div className="flex w-full items-center">
                  ...
                  <div className="w-full ml-6">
                    ...
                    <select
                      ...
                      className={`select  select-bordered  w-1/2`}
                    >
                      <option value="Jom Pay">Bank Transfer</option>
                      <option value="FPX">FPX </option>
                      <option value="Bank Transfer">Bank Transfer</option>
                      <option value="Duit Now">Duit Now</option>
                    </select>
                  </div>
                </div>
                {/* Cara Pembayaran */}
