...
const AllEvaluator_view = () => {
  const { name, evaluator_id, id_program } = useParams();
    ...
    return (
      ...
      >
        ...
        <div className="space-y-4 p-4">
          ...
          <div className="flex items-center">
            <FaHandSparkles className="text-gray-500 mr-2" />
            <p className="text-lg">
              Bidang Spesifik Penilai:{" "}
              <span className="font-medium">
                {bidangSpesifikStr.split(",").map((item, index) => {
                  return (
                    <div
                      className="badge badge-info badge-soft mr-1"
                      key={index}
                    >
                      {item}
                    </div>
      ...
      >
        <div className="flex flex-row justify-between">
          ...
          <button
            ...
            onClick={() => {
              window.location.href = `/senarai-penilai-dalaman/edit-penilai/${evaluator_id}`;
            }}
          >
            <svg
...
export default AllEvaluator_view;
